...
function drawGoofyCharacter(img, char, isMoving = false) {
  ...
  ctx.lineWidth = 3;
  ctx.beginPath();
  ctx.moveTo(char.x - 8, char.y + 40);
  ctx.lineTo(char.x - 8, char.y + 70 + legOffset);
  ctx.stroke();
  ...
  // Body
  ctx.beginPath();
  ctx.moveTo(char.x, char.y + 30);
  ctx.lineTo(char.x, char.y + 55);
  ctx.stroke();

  // Arms
  ctx.beginPath();
  ctx.moveTo(char.x - 20, char.y + 40);
  ctx.lineTo(char.x - 20 - armSwing, char.y + 40);
  ctx.stroke();
  ...
  ctx.save();
  ctx.beginPath();
  ctx.arc(char.x, char.y, char.r, 0, Math.PI*2);
  ctx.clip();
  ctx.drawImage(img, char.x - char.r, char.y - char.r, char.r*2, char.r*2);
  ctx.restore();
}
...
function update() {
  ...
  if (scene !== "cake") {
    ...
    flowerPositions.forEach(f => {
      ctx.fillStyle = "pink";
      ctx.beginPath(); ctx.arc(f.x-5,f.y,5,0,Math.PI*2); ctx.fill();
      ctx.beginPath(); ctx.arc(f.x+5,f.y,5,0,Math.PI*2); ctx.fill();
  ...
  if (npcActive && npcIndex < npcPositions.length) {
    drawGoofyCharacter(meImg, {
      x: npcPositions[npcIndex].x,
      y: npcPositions[npcIndex].y,
      r: 30
    }, false);
  }
